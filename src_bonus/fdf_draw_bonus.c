...
/*   Updated: 2024/03/02 20:08:03 by tforster         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fdf_lines(t_fdf *fdf, float ***batch, int max_y, int max_x)
{
	...
	while (max_y > line)
	{
		...
		while (max_x > col)
		{
			...
			if (max_x > col + 1 && start.point[3] && batch[line][col + 1][3])
				bresenham(fdf, start, init_vrtx(fdf, batch, line, col + 1));
			if (max_y > line + 1 && start.point[3] && batch[line + 1][col][3])
